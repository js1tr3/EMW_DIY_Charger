...
int BMSon=0; // this will be changed in code
float duty=0;
...
void setup() {
  ...
  // get the display going
   #ifdef NEW_DISPLAY_SPE // Don't use auto-baud of uLCD-144-G2
...
#ifdef DEBUG        
        printClrMsg("Wrong charge\n profile!", 5000, 0x1f, 0x3f, 0);
#endif
    return -1;
  }
      ...
      if(BMSon && digitalRead(pin_BMS)==LOW) { // active LOW (so you need to pull up by connecting miniBMS loop to EOC signal)
        ...
        while(1) {
          ...
          if(m>100) {
            // this is for real
#ifdef DEBUG        
            printClrMsg("BMS Stop Signal. \n Exiting in 5 sec...", 5000, 0x1f, 0x3f, 0);
#endif
            stopPWM();          
